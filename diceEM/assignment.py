...
def dice_posterior(sample_draw: List[int], 
    ...
    likelihood_1 = (math.factorial(sum(sample_draw)) /
                  np.prod([math.factorial(x) for x in sample_draw])) *(
                  np.prod([safe_exponentiate(prob_face,count_face) for prob_face, count_face in 
                           zip(dice[0].face_probs, sample_draw)]))
    total_dice = sum(die_type_counts)
    prior_1 = die_type_counts[0] / total_dice
    posterior_1 = likelihood_1*prior_1
    likelihood_2 = (math.factorial(sum(sample_draw)) /
                  np.prod([math.factorial(x) for x in sample_draw])) * (
                      np.prod([safe_exponentiate(prob_face,count_face) for prob_face, count_face in 
                           zip(dice[1].face_probs, sample_draw)]))
    prior_2 = die_type_counts[1] / total_dice
    posterior_2 = likelihood_2*prior_2
    # Normalize
    posterior_normalized_1 = (posterior_1)/(posterior_1+posterior_2)
    return(posterior_normalized_1)
...
def diceEM(experiment_data: List[NDArray[np.int_]],  # pylint: disable=C0103
    ...
    while (((iterations == 0) or
        ...
        bag_of_dice = m_step(expected_counts)
        # Update priors
...
def e_step(experiment_data: List[NDArray[np.int_]],
    ...
    prior0 = bag_of_dice.die_priors
    for draw in experiment_data:
        # Calculate posterior for each die based on the draw
        posteriors = [dice_posterior(draw, prior0, [bag_of_dice.dice[0], bag_of_dice.dice[1]])]
        posteriors.append(1 - posteriors[0])  # posterior_2 = 1 - posterior_1
        for i in range(len(bag_of_dice.dice)):
            expected_counts[i] += posteriors[i] * draw
